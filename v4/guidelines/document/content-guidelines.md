...
- **Persona**: See `guidelines/document/persona.md`.
...
3. **Overview diagram** (Excalidraw or Mermaid SVG) with `<figure>` and `<figcaption>`
4. **Table of Contents** anchor (`## Table of Contents`)
5. **Abstract** section (mental model for the entire article—see below)
6. **Main sections** (H2/H3) with in-depth technical analysis
7. **Conclusion** (concise synthesis)
8. **Appendix** (final H2) containing:
...
The Appendix must be the final H2 section.

### Table of Contents Placement

Every article MUST include a `## Table of Contents` section placed AFTER:
1. The H1 title
2. The description/abstract paragraphs
...
Description paragraph(s) providing mental model...

<figure>

![Overview](./overview.svg)

<figcaption>High-level overview of the concept.</figcaption>
</figure>
...
The Table of Contents section itself should be empty -- it serves as an anchor point. The actual TOC is auto-generated by the site.
...
Use the following skills from the global claude-devkit for content work:

- **`/article <topic>`** - Write new articles with deep research
- **`/article update <path> <prompt>`** - Update existing articles with deep research
- **`/article review <path>`** - Review articles. This should:
  - Generate thumbnail images if missing
  - Migrate inline mermaid diagrams to Excalidraw where appropriate (overview, architecture, flow)
  - Run multi-model review using `/multi` (Claude + Codex/ChatGPT + Gemini optional)
- **`/blog`** - Write blog posts
- **`/project-docs`** - Write or update project documentation from local code
- **`/diagram`** - Standalone diagram generation (delegates to `/excalidraw` or `/mermaid`)
- **`/excalidraw`** - Generate Excalidraw diagrams directly
- **`/mermaid`** - Generate Mermaid diagrams directly
- **`/image-transform`** - Convert between image formats (e.g., SVG to JPEG for thumbnails)

All content skills load guidelines from `guidelines/document/` automatically.
...
- Overview diagram with figcaption (Excalidraw SVG preferred)
- `## Table of Contents` anchor present after overview diagram
...
- `thumbnail.jpg` present in the article folder
